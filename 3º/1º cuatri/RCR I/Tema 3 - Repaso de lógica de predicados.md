...
Forma más básica de lógica, centrada en proposiciones completas.
### Sintaxis
...
2. **Conectivas**: Operadores lógicos:
	- **Negación** ($\neg$)
	- **Conjunción** ($\wedge$)  
	- **Disyunción** ($\vee$)
...
**Simil para entenderlo mejor**
Es querer probar una acusación ($\alpha$). En vez de probar su culpa, asumimos que es inocente ($\neg \alpha$) y lo añade a las evidencias (Δ). Si al combinarlas, incluyendo la de que es inocente, se llega a una contradicción lógica irrefutable, entonces lo de ser inocente es falso y por lo tanto, nuestra acusación original es verdadera.